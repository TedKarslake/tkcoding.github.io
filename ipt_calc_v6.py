...
while question == "y":
    ...
    while True:
        question2 = input("Press 'p' then [Enter key] for Net Premium input, 'g' then [Enter key] for Gross Premium input. Enter 'n' then [Enter key] to stop: ")
    ...
    if  question2 == "p": #'n' was already defined above to exit, so 'p' is for net 'p'remium
        ...
        print("")
        net = float(input("Net Premium (number only): £"))
        try:
            netnum = float(net)
        except ValueError:
            print("")
        ...
        broker_dec = float(broker_float / 100)
        print("")
        gross = input("Gross Premium (number only): £")
        try:
        ...
        False
        print("")
        print("Thank you for using the Insurance Premium Tax Calculator.")
